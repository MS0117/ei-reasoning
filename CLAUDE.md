...
- (IV) training objective (`train.py`) — region-weighted SFT and/or DPO, or the group-advantage objective below

**Group-advantage objective (`train.objective: gadv` — `docs/objective_gadv_spec_20260903.md`).** Builds per-question groups from the iteration's OWN rollouts (`gadv.py`, called by `build_dataset`): frontier questions (1..n−1 of n clean-correct) contribute every clean-correct row at A = 1−k/n plus their failures at negative advantages (`source="wrong"`), rescued cliffs form one group of the n failures + the kept rescues, 8/8 questions are excluded; each question's negative mass equals its positive mass and is split across wrong rows by answer-frequency^γ. `GadvTrainer` trains the PPO-clipped surrogate against the trainer's INITIAL weights (per-token θ₀ log-probs cached by a `TrainerCallback.on_train_begin` pre-pass through `model_wrapped`, exchanged across ranks) with a token-mean over the gathered window; advantages and the cache are looked up by a `row_idx` dataset column because DeepSpeed casts float batch channels to bf16. Mutually exclusive with `train.sft.cliff`; the `sft`/`sft+dpo` paths are untouched. Arm presets live in `configs/methods/arms/*.yaml` and are applied to a frozen run with `scripts/fork_run.py --overlay` (`PRESET=... bash scripts/l3_arm.sh P <FROZEN> <GPUS>`); `configs/methods/l5_gadv.yaml` is the L5 preset (`init_from: last`). Adding config fields changes every existing run's config hash — forks restamp, in-place resumes need `scripts/restamp_config_hash.py`.
