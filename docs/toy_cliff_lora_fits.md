...
| 버전 | 날짜 | 내용 |
|---|---|---|
| **v3 (Part II)** | 2026-09-01 | **라운드 2 — 같은 250 위에서 6스텝·16샘플.** 스텝 4→6의 paired 효과, 예산 재배분 판정 |
| v2 (Part I) | 2026-08-31 | 250-cliff 세트, 5 arm 동일 예산(4스텝·24샘플·어댑터당 ≤100문제), 후보 품질 지표 추가 |
...
## Part II — 라운드 2 @ 같은 250 cliff, 6스텝 · 16샘플 (2026-09-01)

라운드 1의 fit loss가 4스텝에서 아직 선형 하강 중이었고(예: 0.547→0.509), 샘플 17–24는 대조를 바꾸지
않으면서 시간만 먹었다(BRIDGE−LSPO @24 +17.6pp vs @16 +16.8pp, 불일치율 36.8% 동일). 그래서 예산을
**샘플에서 스텝으로 옮겼다**: single arm `fit.steps 4→6, improve.n 24→16`, staged `2+2→3+3, 8+16→8+8`.
`_subset250` 그대로 재사용하므로 **같은 250문항·같은 실패 궤적**이고, 유일한 변수가 예산 배분이다.

### 8. 결론부터 — 예산 재배분은 실패했다

**어느 arm도 유의하게 좋아지지 않았고, 후보 품질은 전 arm에서 뚜렷하게 나빠졌다.** 시간은 26.5h → 22.5h로
줄었지만 그 대가로 학생에게 넘길 데이터의 학습가능성을 잃었다. **라운드 1 예산(4스텝·24샘플)을 유지할 것.**

### 9. 스텝 4→6 paired 효과 (샘플 16으로 통일)

라운드 1을 **앞 16 attempt만** 채점하면 라운드 2와 완전히 같은 샘플 예산이 된다(staged는 stage-1 8개 +
final 앞 8개 = 라운드 2의 8+8과 동일 스케줄). 같은 250문항이므로 McNemar가 성립한다.

| arm | r1 @24 | r1 @16 | **r2 (6스텝·16샘플)** | Δ | b / c | p |
|---|---|---|---|---|---|---|
| CONTROL | 0.260 | 0.184 | 0.200 | +1.6pp | 23 / 19 | 0.644 |
| LSPO | 0.324 | 0.280 | 0.272 | −0.8pp | 33 / 35 | 0.904 |
| BRIDGE | 0.500 | 0.448 | **0.484** | +3.6pp | 41 / 32 | 0.349 |
| STAGED | 0.508 | 0.432 | 0.420 | −1.2pp | 35 / 38 | 0.815 |
| STAGED_DPO | 0.492 | 0.412 | **0.452** | +4.0pp | 41 / 31 | 0.289 |

- **전부 유의하지 않다**(p ≥ 0.29). bridge 계열의 +3.6/+4.0pp가 그나마 크지만 불일치쌍이 70여 개라
  그 크기로는 안 갈린다.
- **CONTROL 행이 바닥 노이즈의 실측값**이다: fit이 없고 config가 동일한데도 재추첨만으로 +1.6pp(p=0.64)
  움직인다. LSPO·STAGED의 −0.8/−1.2pp는 이 노이즈 안이고, BRIDGE·DPO의 +3.6/+4.0pp는 그 2배 남짓이다.
- 즉 **스텝 4→6은 conversion을 사지 못했다.** 라운드 1에서 3→4 스텝이 +9pp를 샀던 것과 대조된다 —
  수확체감 구간에 진입했다는 뜻이다.

### 10. 라운드 2 conversion과 McNemar (N=250)

| arm | conversion | kept |
|---|---|---|
| BRIDGE | **0.484** | 121/250 |
| STAGED_DPO | 0.452 | 113/250 |
| STAGED | 0.420 | 105/250 |
| LSPO | 0.272 | 68/250 |
| CONTROL | 0.200 | 50/250 |

| A vs B | Δ | 불일치 | p |
|---|---|---|---|
| BRIDGE vs CONTROL | +28.4pp | 40.4% | 2.8e-13 |
| STAGED_DPO vs CONTROL | +25.2pp | 39.6% | 1.0e-10 |
| STAGED vs CONTROL | +22.0pp | 35.6% | 3.2e-9 |
| BRIDGE vs LSPO | +21.2pp | 42.0% | 2.2e-7 |
| STAGED_DPO vs LSPO | +18.0pp | 42.0% | 1.3e-5 |
| STAGED vs LSPO | +14.8pp | 40.4% | 3.0e-4 |
| LSPO vs CONTROL | +7.2pp | 28.0% | **0.041** |
| BRIDGE vs STAGED | +6.4pp | 24.0% | 0.052 |
| BRIDGE vs STAGED_DPO | +3.2pp | 33.6% | 0.45 |
| STAGED vs STAGED_DPO | −3.2pp | 27.2% | 0.40 |

- **Part I의 두 계층 구조가 그대로 재현**됐다: bridge 계열이 gold를 +15~21pp, CONTROL을 +22~28pp로 이기고
  bridge 계열끼리는 갈리지 않는다.
- **gold vs CONTROL이 이번엔 p=0.041로 경계를 넘었다**(라운드 1은 +6.4pp, p=0.060). 두 라운드를 합쳐 읽으면
  "gold는 CONTROL보다 약간 낫지만 그 효과는 bridge의 1/3 수준이고 N=250에서 겨우 잡히는 크기"가 정확한
  서술이다. 단일 라운드의 p로 "유의/무의"를 뒤집어 말하지 말 것.
- BRIDGE vs STAGED가 +6.4pp(p=0.052)로 처음 경계에 왔다. 라운드 1에서는 −0.8pp였으므로 **방향이 뒤집혔고**,
  이는 8+8 예산이 staged에 불리하다는 사전 예측(final rollout 16→8이 −7.6pp)과 일치한다. staged의 열세는
  스케줄의 문제가 아니라 **부여한 예산의 문제**로 읽어야 한다.

### 11. 후보 품질 — 스텝을 늘리면 후보가 베이스에서 멀어진다

같은 정의(베이스 정책 아래 스코어링, `C = s_mean + s_tail`, 낮을수록 학습 용이):

| arm | s_mean (r1 → **r2**) | C(y) (r1 → **r2**) | 길이 | ref% | 타깃 base NLL |
|---|---|---|---|---|---|
| CONTROL | 0.354 → 0.361 | 2.76 → **2.83** | 6,526 | 0.0% | — |
| LSPO | 0.440 → **1.286** | 3.44 → **7.03** | 3,788 | 0.9% | 1.238 |
| BRIDGE | 0.449 → **0.636** | 3.73 → **4.91** | 4,134 | 2.3% | 0.517 |
| STAGED | 0.443 → 0.461 | 3.63 → **3.77** | 4,421 | 1.7% | 0.513 |
| STAGED_DPO | 0.336 → 0.370 | 2.88 → **3.13** | 4,011 | 0.4% | 0.536 |

**이것이 라운드 2의 핵심 발견이다.** CONTROL(fit 없음)은 2.76→2.83으로 사실상 불변인데, fit이 있는 arm은
전부 나빠졌고 **스텝이 실제로 늘어난 정도에 비례한다**:

- **LSPO가 파국적이다: C 3.44 → 7.03 (2배).** gold는 타깃 base NLL이 1.238로 원래 멀었는데, 6스텝을 주면
  어댑터가 그쪽으로 끌려가 **후보의 s_mean이 1.286 — 타깃의 NLL과 같아진다.** 4스텝에서는 0.440으로
  타깃(1.238)보다 훨씬 on-policy했다. 즉 **스텝이 늘면 어댑터가 타깃을 복제하기 시작하고, 타깃이 멀면
  후보도 멀어진다.** conversion은 전혀 안 늘었으므로(−0.8pp) 순손실이다.
- BRIDGE도 같은 방향(3.73 → 4.91)이지만 타깃이 가까워서(0.517) 피해가 작다.
- **staged 계열이 가장 안정적이다**(3.63→3.77, 2.88→3.13). 스텝이 2+2→3+3으로 나뉘어 각 fit이 짧기 때문.

**Part I §7-2의 DPO 소견은 라운드 2에서도 유지된다:**

| | stage-1 (SFT) | stage-2 |
|---|---|---|
| STAGED | s_mean 0.360, C 3.03 | **0.582, C 4.65** (SFT 3스텝) |
| STAGED_DPO | s_mean 0.385, C 3.19 | **0.353, C 3.05** (DPO 3스텝) |

paired: 전체 공통 75문제 ΔminC(SFT−DPO) **+0.358** (48/27, p=0.020), stage-2에서 양쪽 전환한 21문제는
**+1.051 (21/0, p=9.5e-7)**. 라운드 1(+0.56 / +0.95)과 같은 크기·같은 방향이다. **SFT는 스텝을 더 주면
후보를 밀어내고(3.03→4.65) DPO는 당긴다(3.19→3.05)** — 라운드 1보다 격차가 더 벌어졌다.

attractor 질량(stage-2가 stage-1 최빈오답에 남긴 것): STAGED **0.390** / STAGED_DPO **0.426**.
라운드 1(0.476 / 0.449)보다 낮아졌지만 여전히 밴드(0.39~0.50) 안이고 두 목적함수 간 순서가 뒤집혔다 —
**목적함수로는 안 움직인다**는 결론이 세 번째로 확인됐다.

### 12. 판정과 다음

| | |
|---|---|
| **예산** | **4스텝 · 24샘플로 복귀.** 6스텝은 conversion을 못 사면서 C(y)를 크게 악화시킨다. 특히 LSPO는 쓰면 안 된다 |
| **스텝 축** | 3→4는 +9pp, 4→6은 0pp. **수확체감이 4 근처에서 시작**한다. 8스텝은 시험할 이유가 없다 |
| **샘플 축** | 16→24가 +4~5pp를 사고 대조는 안 바꾼다. conversion 절대값이 필요하면 샘플, 대조만 필요하면 16으로 충분 |
| **staged 예산** | 8+8은 staged에 불리하다(BRIDGE vs STAGED가 −0.8 → +6.4pp로 반전). 비교하려면 8+16 유지 |
| **아직 미측정** | bridge 궤적 자체의 C(y)/s_tail(스코어링 패스 필요, ~15분), `d_tail`(어댑터-베이스 괴리, `gamma_dtail>0`), C 기반 선택(`selection.method: c_score`)의 효과 |

라운드 2 런: `runs/toy_cliff_2/default_{CONTROL_20260831_070453, LSPO_20260831_124437,
BRIDGE_20260831_164407, STAGED_20260831_222601, STAGED_DPO_20260901_043119}`.
소요 22h32m(CONTROL 2h24 / LSPO 3h59 / BRIDGE 5h41 / STAGED 6h05 / DPO 6h25).
